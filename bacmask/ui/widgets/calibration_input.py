...
class CalibrationInput(BoxLayout):
    def __init__(self, service: MaskService, **kwargs: Any) -> None:
        ...
        self.add_widget(Label(text="mm/px:", size_hint_x=None, width=60))
        self.mm_per_px = TextInput(multiline=False, write_tab=False, hint_text="e.g. 0.0125")
        self.mm_per_px.bind(on_text_validate=self._commit_mm_per_px)
        self.add_widget(self.mm_per_px)

        self.add_widget(Label(text="px/mm:", size_hint_x=None, width=60))
        self.px_per_mm = TextInput(multiline=False, write_tab=False, hint_text="e.g. 80")
